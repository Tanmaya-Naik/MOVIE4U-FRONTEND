...
const MainMovieList = () => {
  ...
  const displayedMovies = useMemo(() => {
    ...
    if (minRating > 0) {
      list = list.filter((m) => (m.vote_average ?? m.rating ?? 0) >= minRating);
    }
    ...
    if (sort.by !== "default") {
      if (sort.by === "release_date") {
        ...
        list.sort(
          (a, b) =>
            ((a[sort.by] ?? a.rating ?? 0) - (b[sort.by] ?? b.rating ?? 0)) *
            (sort.order === "asc" ? 1 : -1)
        );
      }
    }
      ...
      <header className="flex justify-between items-center mb-8 w-full px-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <select
            ...
            value={sort.by}
            onChange={handleSort}
            className="rounded px-2 py-2 bg-black text-white border border-gray-700 text-xl "
          >
